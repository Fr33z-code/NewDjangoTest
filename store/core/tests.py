...
from catalog.models import ProductCatalogTests, Category

class ProductCatalogTests(TestCase):
    def setUp(self):
        self.category = Category.objects.create(name="Шубы", slug="coat")

        self.product1 = ProductCatalogTests.objects.create(
            name="Шуба из норки",
            slug="mink-coat",
            price=50000,
            in_stock=True,
        ...
        self.product2 = ProductCatalogTests.objects.create(
            name="Шуба из овчины",
            slug="sheepskin-coat",
            price=30000,
            in_stock=False,
    ...
    def test_filter_by_category(self):
        response = self.client.get('/catalog/?category=coat')
        self.assertContains(response, "Шуба из норки")
        self.assertContains(response, "Шуба из овчины")
